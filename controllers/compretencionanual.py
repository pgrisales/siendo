...
def index():
    ...
    tarea=0
    modulos=db(db.tbl_modulo.ambiente=='retencionanual.amb').select().first()
    if modulos==None:
        raise HTTP(404)
    ...
    if formulario.process().accepted:
        ...
        if Verificar(formulario.vars.archivo):
            tarea=db.tbl_control_maestro.insert(
                ...
                funcion="Crear Pdf",
                args="{" + f"'archivo': '{formulario.vars.archivo}','modulo':'retencionanual.amb'" + "}",
                estado= 'I')
        else:
            db.tbl_mensajes.insert(mensaje=f"Archivo:[{formulario.vars.archivo}] no es un Comprobante retención anual!")
            msgerror=f"Archivo:[{formulario.vars.archivo}] no es un Comprobante de retención anual!"
...
def BuscarCorreoEnviado():
    modulos=db(db.tbl_modulo.ambiente=='retencionanual.amb').select().first()
    return (BuscarEnClientes(modulos.id))
...
def fun_pendiente(estado,titulo,color="bg-primary",idtabla="",botones=None):
    ...
    consulta &=db.tbl_recepcion.tipdoc==modulos.id
    ...
    if estado == "E":
        tabla.append (THEAD(TR(
                        TH(),TH("Nro.Doc"),TH("Fecha"),TH("NIT"),TH("Proveedor"),TH("Valor"),TH("Accion")
                        )))
    elif estado == "C":
        tabla.append (THEAD(TR(
...
#ajax
def ReenvioCorreo():
    id_recepcion=request.vars.id_recepcion or None
    if not id_recepcion: return "alert('Error, no exite Proveedor');"
    recepcion=db(db.tbl_recepcion.id==id_recepcion).select().first()
    if recepcion==None:        return "Alert ('error no existe recepcion');"


    salida=DIV()
    salida.append(fun_input("remail","Email","email@domininio.com",tipo="email", valor=recepcion.id_cliente.correo1))
    ...
    botones =XML(A("Reenviar",_class="btn btn-success text-white",
                    _onclick="ajax('{}',['remail','cambiar'],':eval');".format(URL("ReenvioCorreoGuadar",vars=dict(id_recepcion=id_recepcion)))
                    ))
...
def ReenvioCorreoGuadar():
    ...
    consulta=db(db.tbl_recepcion.id==id_recepcion).select().first()
    if cambiar:
        db(db.tbl_cliente.id==consulta.id_cliente).update(correo1=correo)


    tarea = planificador.queue_task(fun_EnviarCorreo,
                                    pvars=dict(envio = id_recepcion, correo=correo,paraserver=VerificarModulo("retencionanual.amb")),
                                    timeout = 360)
    if tarea:
        consulta.tarea=tarea.id
        consulta.update_record()
        salida+=f"alert('Correo Enviado');"
...
def enviocorreo():
    ...
    for envio in envios:
        tarea = planificador.queue_task(fun_EnviarCorreo,
                                        pvars=dict(envio = envio,paraserver=VerificarModulo("retencionanual.amb")),
                                        timeout = 360)
