...
def index():
    ...
    tarea=0
    modulos=db(db.tbl_modulo.id==2).select().first()
    if modulos==None:
        raise HTTP(404)
    ...
    if formulario.process().accepted:
        ...
        if Verificar(formulario.vars.archivo):
            ...
            #                             pvars=dict(archivo = formulario.vars.archivo,modulo=2),
            #                             )#timeout = 360)
            # tarea = tarea.id
        else:
            db.tbl_mensajes.insert(mensaje=f"Archivo:[{formulario.vars.archivo}] no es un Comprobante de Nomina!")
            msgerror=f"Archivo:[{formulario.vars.archivo}] no es un Comprobante de Nomina!"
...
def fun_pendiente(estado,titulo,color="bg-primary",idtabla="",botones=None):
    ...
    consulta &=db.tbl_recepcion.tipdoc==2
...
def ReenvioCorreo():
    ...
    if not id_recepcion: return "alert('Error, no exite Empleado');"
...
def ReenvioCorreoGuadar():
    ...
    tarea = planificador.queue_task(fun_EnviarCorreo,
                                    pvars=dict(envio = id_recepcion, correo=correo,paraserver=VerificarModulo("compnomina.amb")),
                                    timeout = 360)
    if tarea:
...
def enviocorreo():
    ...
    for envio in envios:
        tarea = planificador.queue_task(fun_EnviarCorreo,
                                        pvars=dict(envio = envio,  paraserver=2),
                                        timeout = 360)
