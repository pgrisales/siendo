...
def plugin_main(*args, **kwargs):
    # print ('args  : ',args)
    # print ('kwargs: ',kwargs)
    if not 'campo' in kwargs:
        return None
    if kwargs['campo'] in kwargs['objPagina'].campos:
        valor =kwargs['objPagina'].campos[kwargs['campo']]['texto'][0]
        valor =valor.replace('$','')
        valor =valor.replace('.','')
        valor =valor.replace(',','.')
        #print (valor , type (valor))
        valor = float (valor)
        valor = numero_a_moneda ( valor )
        kwargs['objPagina'].campos[kwargs['campo']]['texto']=[valor.upper()]
